...
test("chat, image, video and detail pages stay usable", async ({ page, context }) => {
  ...
  await expect(page.getByText("E2E 聊天连通性测试")).toBeVisible();
  await expect(page.getByText(/我已收到你的请求：E2E 聊天连通性测试/)).toBeVisible();
  ...
  await expect(page.getByText(/正在生成图片|图片已生成/)).toBeVisible();
  ...
  await expect(page.getByText("视频任务已提交")).toBeVisible();
});

// 验证新建会话不会把上一个会话的图片请求展示状态清空。
test("new conversation does not erase previous image request display", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: /图片/ }).click();
  await page.locator("textarea").fill("E2E 会话隔离图片请求");
  await page.getByRole("button", { name: "发送" }).click();
  await expect(page.getByText("E2E 会话隔离图片请求")).toBeVisible();
