...
export function PipelineDagView({ nodes, dependencies }: Props) {
  ...
  return (
    <div role="region" aria-label="任务依赖 DAG 图" tabIndex={0} style={{ overflow: "auto", border: "1px solid hsl(var(--foreground) / 0.12)", borderRadius: 10 }}>
      <svg width={width} height={height} role="img" aria-labelledby="dag-title dag-desc">
        ...
        <defs><marker id="dag-arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="var(--text-muted)" /></marker></defs>
        {dependencies.map((edge) => {
          const from = positions.get(edge.source_node_id); const to = positions.get(edge.target_node_id);
          if (!from || !to) return null;
          return <g key={edge.id}>
            <path d={`M ${from.x + 180} ${from.y + 30} C ${from.x + 205} ${from.y + 30}, ${to.x - 25} ${to.y + 30}, ${to.x} ${to.y + 30}`} fill="none" stroke="var(--text-muted)" strokeWidth="2" markerEnd="url(#dag-arrow)" />
            {edge.condition_type && edge.condition_type !== "always" ? <text x={(from.x + to.x + 180) / 2} y={(from.y + to.y) / 2 + 20} textAnchor="middle" fill="var(--text-secondary)" fontSize="10">{edge.condition_type}</text> : null}
          </g>;
        })}
        {nodes.map((node) => {
          const pos = positions.get(node.id)!;
          const statusColor = node.status === "completed" ? "hsl(var(--success))" : node.status === "failed" ? "hsl(var(--destructive))" : node.status === "running" ? "hsl(var(--info))" : "var(--text-muted)";
          return <g key={node.id} role="listitem" aria-label={`${node.name || node.id}，${node.status}`}>
            <rect x={pos.x} y={pos.y} width="180" height="62" rx="8" fill="var(--bg-card)" stroke={statusColor} strokeWidth="2" />
            <circle cx={pos.x + 16} cy={pos.y + 18} r="5" fill={statusColor} />
            <text x={pos.x + 28} y={pos.y + 22} fill="var(--text-primary)" fontSize="12">{(node.name || node.id).slice(0, 20)}</text>
            <text x={pos.x + 16} y={pos.y + 45} fill="var(--text-secondary)" fontSize="10">{node.type} · {node.status}</text>
          </g>;
        })}
      </svg>
    </div>
