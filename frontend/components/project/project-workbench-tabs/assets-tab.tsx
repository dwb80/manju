    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-muted-foreground">素材统一管理，点击复用可直接带入生成输入区。</div>
        </div>
        <div className="flex flex-wrap gap-2">
          <input
            className="h-8 w-40 rounded-lg border border-border bg-muted px-3 text-xs text-foreground outline-none focus:border-primary"
            value={assetSearch}
            placeholder="搜索资产"
            onChange={(event) => setAssetSearch(event.target.value)}
          />
          <input
            className="h-8 w-32 rounded-lg border border-border bg-muted px-3 text-xs text-foreground outline-none focus:border-primary"
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 flex-wrap">
        <button
          className={`shrink-0 rounded-lg border px-4 py-2.5 text-xs ${assetKindFilter === "all" ? "border-primary bg-primary/10 text-primary" : "border-border bg-muted text-muted-foreground hover:bg-muted hover:border-border"}`}
        ...
        {projectAssetKinds.map((item) => (
          <button
            key={item.key}
            className={`shrink-0 rounded-lg border px-4 py-2.5 text-xs ${assetKindFilter === item.key ? "border-primary bg-primary/10 text-primary" : "border-border bg-muted text-muted-foreground hover:bg-muted hover:border-border"}`}
            onClick={() => {
      ...
      <div className="rounded-lg border border-border bg-muted p-3">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <div className="mt-1 text-xs text-muted-foreground">用表单维护资产名称、标签、媒体地址、提示词和特征信息。</div>
          </div>
          <div className="flex flex-wrap gap-2">
        ...
        <div className="grid grid-cols-2 gap-2 max-md:grid-cols-1">
          ...
          />
          <input
            className="h-9 rounded-lg border border-border bg-secondary px-3 text-xs text-foreground outline-none focus:border-primary"
        ...
        <textarea
          className="mt-2 min-h-20 w-full resize-none rounded-lg border border-border bg-secondary px-3 py-2 text-xs text-foreground outline-none focus:border-primary"
      ...
      {filteredProjectAssets.length === 0 ? (
        projectAssets.length === 0 ? (
          <div className="px-5 py-10 text-center">
            <div className="text-base font-semibold text-foreground">暂无资产</div>
            <div className="mt-2 text-sm text-muted-foreground">资产库用于管理角色卡、场景卡、风格模板等素材，可在分镜中快速复用。</div>
          </div>
        ) : (
        ...
        <ProjectManagementTable columns={["预览", "名称", "类型", "标签/文件夹", "媒体信息", "操作"]}>
          {pagedProjectAssets.map((asset) => (
            <tr key={asset.id} className="group align-top hover:bg-muted/40 transition-all duration-200">
              <td className="w-40 px-4 py-4">
                {asset.image_url ? (
                  <button className="block h-24 w-40 overflow-hidden rounded-lg bg-black/30 transition-all duration-200 hover:scale-[1.02] hover:ring-2 hover:ring-primary/50" onClick={() => window.open(asset.image_url, "_blank", "noopener,noreferrer")}>
                  ...
                  <div className="grid h-24 w-40 place-items-center rounded-lg border border-dashed border-border bg-secondary text-muted-foreground">无预览</div>
                )}
              </td>
              <td className="max-w-72 px-3 py-3">
                <div className="font-semibold text-foreground">{asset.name || "未命名资产"}</div>
                {asset.prompt && <div className="mt-1 line-clamp-2 whitespace-pre-wrap text-foreground/80">{asset.prompt}</div>}
              </td>
              <td className="px-3 py-3 text-foreground/90">
                {projectAssetKinds.find((item) => item.key === asset.kind)?.label ?? asset.kind}
                {asset.is_favorite && <span className="ml-2 rounded-md bg-chart-5/10 px-1.5 py-0.5 text-chart-5">收藏</span>}
              </td>
              <td className="max-w-64 px-3 py-3 text-foreground/80">
              ...
              <td className="w-56 px-3 py-3">
                <div className="flex flex-wrap gap-1">
                  ...
                  <button className={`grid h-8 w-8 place-items-center rounded-md bg-muted hover:bg-muted ${asset.is_favorite ? "text-chart-5" : "text-foreground/80"}`} onClick={() => void toggleProjectAssetFavorite(asset)} aria-label="收藏资产">
                    <Star className={`h-3.5 w-3.5 ${asset.is_favorite ? "fill-current" : ""}`} />
                  </button>
                  <button className="grid h-8 w-8 place-items-center rounded-md bg-destructive/10 text-destructive hover:bg-destructive/20" onClick={() => void deleteProjectAssetItem(asset)} aria-label="删除资产">
