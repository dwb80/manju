...
export const DialogContent = React.forwardRef<React.ElementRef<typeof Primitive.Content>, React.ComponentPropsWithoutRef<typeof Primitive.Content> & { size?: "default" | "wide" }>(({ className, children, size = "default", ...props }, ref) => <DialogPortal><DialogOverlay /><Primitive.Content ref={ref} className={cn("fixed left-1/2 top-1/2 z-50 grid w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl border border-white/10 bg-[#141414] p-6 shadow-2xl", size === "wide" && "max-w-4xl", className)} {...props}>{children}<Primitive.Close className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-md text-foreground/60 transition-colors hover:bg-white/10 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/40"><X className="h-4 w-4" /><span className="sr-only">关闭</span></Primitive.Close></Primitive.Content></DialogPortal>);
