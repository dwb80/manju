...
export function Pagination({
  ...
  return (
    <div className="flex flex-nowrap items-center justify-between gap-3 rounded-lg border border-white/10 bg-[#252525] px-4 py-3 overflow-x-auto">
      {/* 左侧：总数、每页数量、快速跳转 */}
      <div className="flex flex-nowrap items-center gap-4 shrink-0">
        {showTotal && (
          <div className="text-sm text-[#888] whitespace-nowrap">
        ...
        {showPageSize && onPageSizeChange && (
          <div className="flex items-center gap-2 whitespace-nowrap">
            <span className="text-sm text-[#888]">每页</span>
            <select
              value={pageSize}
              onChange={handlePageSizeChange}
              className="rounded border border-white/10 bg-[#1a1a1a] px-2 py-1 text-sm text-white focus:border-emerald-500/50 focus:outline-none focus:ring-1 focus:ring-emerald-500/50"
            >
            ...
            <span className="text-sm text-[#888]">条</span>
          </div>
        )}
        ...
        {showQuickJumper && (
          <div className="hidden md:flex items-center gap-2 whitespace-nowrap">
            <span className="text-sm text-[#888]">跳至</span>
            <input
              type="number"
              min={1}
              max={totalPages}
              ...
              className="w-16 rounded border border-white/10 bg-[#1a1a1a] px-2 py-1 text-sm text-white focus:border-emerald-500/50 focus:outline-none focus:ring-1 focus:ring-emerald-500/50"
              placeholder={currentPage.toString()}
            />
            <span className="text-sm text-[#888]">页</span>
            <button
              onClick={handleJump}
              className="rounded border border-white/10 bg-[#1a1a1a] px-3 py-1 text-sm text-white transition-colors hover:bg-white/10"
            >
              确定
            </button>
          </div>
      ...
      <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="flex sm:hidden items-center gap-2 px-2">
            <span className="text-sm text-emerald-400 font-medium">{currentPage}</span>
            <span className="text-sm text-[#888]">/ {totalPages}</span>
          </div>
