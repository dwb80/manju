...
export function OutlineView({
  ...
  const renderNode = (node: OutlineNode, depth: number = 0) => {
    ...
    return (
      <div key={node.id} className="outline-node">
        <div
          className={`flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer transition-colors ${dropTarget === node.id ? 'bg-primary/20' : 'hover:bg-muted/50'
            }`}
        ...
        >
          {/* 拖拽手柄 */}
          <GripVertical className="h-3 w-3 text-muted-foreground cursor-move flex-shrink-0" />
          ...
          <div className="flex-1 min-w-0">
            ...
            {node.subtitle && (
              <span className="text-xs text-muted-foreground ml-2 truncate">{node.subtitle}</span>
            )}
          </div>
  ...
  return (
    <div className="outline-view bg-card rounded-lg border border-border overflow-hidden h-full flex flex-col">
      {/* 标题栏 */}
      <div className="p-3 border-b border-border flex items-center justify-between flex-shrink-0 gap-2">
        <h3 className="text-sm font-medium text-foreground">大纲视图</h3>
        <div className="flex items-center gap-1">
          ...
          {onBackToEdit && (
            <Button
              ...
              className="h-7 px-2 ml-1 text-xs bg-primary hover:bg-primary/90"
              title="返回编辑器"
            >
