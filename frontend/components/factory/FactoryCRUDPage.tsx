...
export function FactoryCRUDPage<TEntity extends FactoryEntity>(props: FactoryCRUDPageProps<TEntity>) {
  // ===== 客户端 mount 守卫 =====
  // zustand persist 的 selectedProjectId 在 SSR 时为 ""，CSR 从 localStorage 读出真实值，
  // 直接渲染会导致 hydration mismatch。这里在 hasMounted=false 时返回与 SSR 一致的占位。
  ...
  const statConfigs = useMemo(
    () => (showStats === false ? [] : stats(items)),
    [items, stats, showStats],
  );
...
/**
 * 根据 entityLabel 推导 entityType
 * @description 用于 TemplateSelector 的类型 chip 渲染，将中文名称映射为类型标识
 * @param {string} label - 实体中文名称（如"角色"、"场景"、"道具"）
 * @returns {TemplateEntityType} 对应的类型标识
 */
