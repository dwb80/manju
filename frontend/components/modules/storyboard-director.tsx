...
import { Film, Pencil, Trash2, CheckSquare } from "lucide-react";
...
const config: FactoryCRUDPageProps<Storyboard> = {
  ...
  renderCard: (sb, actions) => {
    ...
    const color = STORYBOARD_STATUS_COLORS[status as keyof typeof STORYBOARD_STATUS_COLORS] ?? "bg-gray-500/20 text-gray-400";
        ...
        <div className="flex items-start gap-3 pl-7">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              {sb.scene_id && <span className="text-xs text-[#888]">场景: {sb.scene_id}</span>}
            </div>
...
export function StoryboardDirectorPage() {
  return <FactoryCRUDPage<Storyboard> {...config} />;
}
