...
export function ImageGeneratorPreview({
  ...
  if (isGenerating) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-4 text-muted-foreground border border-dashed border-border rounded-lg">
        <Loader2 className="h-8 w-8 animate-spin text-primary" />
        <div className="text-sm">AI 正在生成 {n} 张图片，请稍候…</div>
        <div className="text-[11px] text-muted-foreground">预计耗时 {estimatedSeconds}s（最多 2min）</div>
        <Button
          type="button"
          variant="outline"
          size="sm"
          onClick={onCancel}
          className="mt-2 border-border text-muted-foreground hover:text-foreground hover:border-destructive/60 hover:text-destructive gap-1.5"
        >
  ...
  if (candidates.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground border border-dashed border-border rounded-lg">
        <ImageIcon className="h-12 w-12 mb-3 opacity-50" />
        <span className="text-sm">填写提示词后点击「{isImg2Img ? "图生图生成" : "生成图片"}」</span>
        {isImg2Img && (
          <span className="mt-2 text-xs text-warning/80">已加载参考图，将作为图生图输入</span>
        )}
      </div>
    );
  }
      ...
      {candidates.map((item, idx) => {
                ...
                {[0, 1, 2, 3].map((gridIdx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <div className="absolute top-2 left-2 px-2 py-0.5 rounded bg-black/60 text-[10px] text-foreground/90">
                        {pos.label}
                      </div>
                      <button
                        type="button"
                        ...
                        className={`absolute top-2 right-2 h-7 w-7 rounded-full flex items-center justify-center transition-all ${gridSelected
                          ...
                          : "bg-black/60 text-foreground/70 hover:bg-black/80 border border-border"
                          }`}
                      >
        ...
        return (
          <div
            ...
            className={`group relative overflow-hidden rounded-lg border-2 transition-all cursor-zoom-in focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 ${isApplied
          ...
          >
            <img
              src={url}
              alt={`候选图 ${idx + 1}`}
              className="max-h-full max-w-full object-contain bg-card"
            />
            ...
            <button
              ...
              className="absolute bottom-2 right-2 inline-flex items-center gap-1 rounded-md bg-black/75 px-2.5 py-1.5 text-[11px] font-medium text-primary-foreground opacity-0 shadow-lg backdrop-blur transition-all group-hover:opacity-100 hover:bg-primary/90 focus-visible:opacity-100"
            >
              <Maximize2 className="h-3 w-3" />
              查看原图
            </button>
            {isApplied ? (
              <div className="absolute top-2 right-2 inline-flex items-center gap-1 rounded-full bg-primary/95 px-2.5 py-1 text-[10px] font-medium text-primary-foreground shadow">
            ...
            <div className="absolute bottom-2 left-2 px-2 py-0.5 rounded bg-black/60 text-[10px] text-foreground/90 inline-flex items-center gap-1">
              <span>#{idx + 1}</span>
              <span className="text-muted-foreground">·</span>
              <span className="text-primary">{itemRatio}</span>
            </div>
          </div>
        );
      })}
