...
export async function listProjectScripts(ctx: AppContext, projectId: string): Promise<ProjectScript[]> {
  ...
  return scripts.sort((left, right) => left.episode - right.episode || left.created_at.localeCompare(right.created_at));
}
...
export async function breakdownProjectScript(ctx: AppContext, projectId: string, input: ScriptBreakdownInput): Promise<import("../../types.js").ProjectStoryboard[]> {
  ...
  for (const [index, draft] of drafts.slice(0, 80).entries()) {
    ...
    const matchedCharacterIds = characterAssets
      .filter((asset) => characters.some((name) => asset.name.includes(name) || name.includes(asset.name)) || script.includes(asset.name))
      .map((asset) => asset.id);
    const location = draft.location?.trim() || "";
    const matchedScene = sceneAssets.find((asset) => (location && (asset.name.includes(location) || location.includes(asset.name))) || draft.description?.includes(asset.name));
