...
export async function listProjectAssets(ctx: AppContext, projectId: string, filters: { kind?: string | null; q?: string | null; tag?: string | null; favorite?: string | null } = {}): Promise<ProjectAsset[]> {
  ...
  if (filters.favorite === "true") assets = assets.filter((asset: ProjectAsset) => asset.is_favorite);
  if (filters.tag) assets = assets.filter((asset: ProjectAsset) => (asset.tags ?? []).includes(filters.tag ?? ""));
  if (filters.q) {
    const keyword = filters.q.trim().toLowerCase();
    if (keyword) {
      assets = assets.filter((asset: ProjectAsset) => [
        asset.name,
        asset.prompt,
        asset.notes,
        asset.folder,
