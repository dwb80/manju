...
> 本节是 `character_image_history` / `characters` 表字段的对外契约。代码与本文档必须严格一致。
...
| 关系 | `relationships` |  | json | 人物关系 JSON 数组 |
| 出现 | `first_appearance` |  | string | 首次出现（EP01-Scene01） |
| 出现 | `dialogue_count` |  | number | 对白数量 |
| AI | `generation_prompt` |  | text | 生图提示词（用于 AI 生图的标准化提示词） |
...
- **状态**：已设为资产（`is_applied=1`，不会被自动清理）/ 历史（`is_applied=0`，**V1.2 起不做自动裁剪**；上限由用户硬件决定，单实体历史条数无上限——见 [factories-assets-and-image-views.md §11/§13](./factories-assets-and-image-views.md)）
- **主图**：一个角色最多 1 张主图

完整 API 与字段见 [factories-assets-and-image-views.md](./factories-assets-and-image-views.md)。
...
| AI 扩展 | `suitable_shots` |  | string | 适用镜头 |
| AI 扩展 | `reusable_elements` |  | string | 可复用元素 |
| AI 扩展 | `generation_prompt` |  | text | 生图提示词 |
...
| AI 扩展 | `camera_usage` |  | string | 镜头使用建议 |
| AI 扩展 | `generation_prompt` |  | text | 生图提示词 |
| AI 扩展 | `first_appearance` |  | string | 首次出现 |
| AI 扩展 | `confidence` |  | enum | confirmed / inferred |
