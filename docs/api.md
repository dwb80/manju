...
- 视频是异步任务；状态为 `pending / processing / success / failed`（实际代码值，对应 [glossary.md](ddd/glossary.md) 领域术语映射：`pending` → 排队中 `queued`、`processing` → 生成中 `generating`、`success` → 已完成 `completed`、`failed` → 已失败 `failed`）
...
- **版本与契约路径的关系**：[requirements §5.6](requirements-and-acceptance.md#56-核心接口契约) 定义的契约路径为权威目标；版本段叠加在契约路径之上，迁移时一并补齐版本段（如 `POST /api/v1/episodes/{id}/scripts`）。
...
| 主要接口 | 说明 |
|---|---|
| `/api/script-documents`、`/api/script-episodes`、`/api/script-scenes`、`/api/script-dialogues`、`/api/script-comments`、`/api/versions` | 结构化剧本、评论与版本。契约路径见 [§5.1.3](requirements-and-acceptance.md#513-post-apiepisodesidscripts) |
| `POST /api/ai/script-analyze`、`/api/ai/script-generate`、`/api/ai/script-optimize` | 分析、生成、优化；失败保持真实失败态。契约路径见 [§5.1.4](requirements-and-acceptance.md#514-post-apiscriptsidanalyze) |
...
| 主要接口 | 说明 |
|---|---|
| `/api/characters`、`/api/scenes`、`/api/props` | 角色、场景、道具 CRUD。契约路径见 [§5.1.7](requirements-and-acceptance.md#517-post-apiprojectsidcharacters)、[§5.1.8](requirements-and-acceptance.md#518-post-apiprojectsidscenes) |
| `/api/characters/:id/images`、`/api/scenes/:id/images`、`/api/props/:id/images` | 每个实体的多视图图片：GET（按景别/角度/视图类型筛选）/ POST / PATCH / DELETE / PUT primary / POST apply（标记为资产）。完整契约见 [factories-assets-and-image-views.md](./factories-assets-and-image-views.md) |
...
| 主要接口 | 说明 |
|---|---|
| `GET/POST /api/reviews`、`GET /api/reviews/stats`、`POST /api/reviews/:id/approve`、`/reject` | 审核队列和状态流。契约路径见 [§5.1.11](requirements-and-acceptance.md#511-post-apireviews)、[§5.1.12](requirements-and-acceptance.md#512-put-apireviewsid) |
...
| `/api/publish/videos`、`GET/POST /api/publish/plans`、`PUT/DELETE /api/publish/plans/:id` | 成片和发布计划；不等于第三方自动发布。契约路径见 [§5.1.20](requirements-and-acceptance.md#520-post-apipublish-plans)、[§5.1.21](requirements-and-acceptance.md#521-post-apipublish-plansidexecute) |
...
> 以下列出当前实际实现路径与 [requirements §5.6](requirements-and-acceptance.md#56-核心接口契约) 契约路径的差异。**契约路径为权威目标**，实际实现的迁移状态见 [feature-status.md](feature-status.md)。
...
| 剧本生成/优化 | （契约未定义独立路径） | `POST /api/ai/script-generate`、`POST /api/ai/script-optimize` | 实际实现额外提供了生成和优化端点，契约未覆盖 |
