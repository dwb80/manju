...
> **所属上下文**: 资产库上下文（详见 [DDD 文档 §3.4](../../domain/01-domain-requirements-spec.md)）；聚合根为 `CharacterImageHistory` / `SceneImageHistory` / `PropImageHistory`，归属于 `Character` / `Scene` / `Prop` 聚合的引用对象集合。
...
- [ ] 文档：`docs/engineering/03-api.md` / `docs/requirements/modules/06-asset-library.md` / `docs/implementation/02-feature-status.md` 同步更新。
...
> 与 `05-ai-image-config.md §6 多模型适配器契约` 配套。本节只讲"三个工厂如何在不写厂商特定代码的前提下使用多模型"。
...
### 14.8 落地顺序（与 `05-ai-image-config.md §6.8` 配合）
