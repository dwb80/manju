...
| `StartShotGeneration` | ready | `ShotGenerationStarted` | 冻结生成输入（当前构图/图层/文字/特效/动态、Prompt、模型与资产版本）并触发 AI 视频生成 |
...
| `ShotPresentationFrozen` | shotId, snapshotId, snapshotHash, shotVersion | 审核质量、后期制作（只引用冻结快照） |
