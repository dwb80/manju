...
> **规则**：以下术语在代码、文档、UI、API 中统一使用。禁止使用“禁止别名”列中列出的称呼。
> **配套规范**：[DDD 治理规范](02-governance.md)
...
| 创作任务 | ScriptCreationTask | AI 辅助剧本创作的异步任务，覆盖生成/优化/改写/拆场景/拆分镜 | 创作请求、AI写作 |
| 创作候选 | CreationCandidate | AI 创作任务的生成结果，为候选不直接覆盖原文，需用户显式采纳 | 生成结果、AI输出 |
| 采纳 | Adopt | 用户确认将创作候选应用到剧本内容 | 应用、确认 |
...
| 术语 | 定义 | 禁止误用 |
|---|---|---|
| `ProductionDependencyEdge`（生产依赖边） | 连接两个确切对象版本或不可变哈希的来源证据，记录依赖类型和事实事件 | 不得连接 `latest`、可变 URL 或无版本对象 |
| `ImpactAssessment`（影响评估） | 对一次上游变化所涉及直接/递归下游、原因、严重度、完整性和投影水位的只读结论 | 不等于已经执行升级或返工 |
| `DependencyFreshness`（依赖新鲜度） | `current / potentially_stale / stale / blocked` 的读模型叠加属性 | 不替换业务聚合生命周期状态；与 Pipeline 节点超时的 `stale` 语义分开 |
| `TraceabilityChain`（追溯链） | 从制品/候选沿依赖边查询确切输入、AI 执行、采纳、审核、权利和发布证据 | 不以当前对象值猜测历史输入 |
| `projectionWatermark`（投影水位） | 影响/追溯投影已处理到的事件位置与时间，用于声明结果完整性 | 不代表源上下文事务时间或绝对实时 |
| `ProductionPlan`（生产计划） | 版本化的 Episode 阶段目标、责任、时间和容量假设 | 不拥有实际生产进度，不得直接改 Shot/任务/审核状态 |
| `MediaCandidate`（媒体候选） | 与 Shot、确切媒体版本、生成意图和执行快照关联的图片/视频候选 | 任务成功不等于候选已采纳；URL 不是候选业务标识 |
| `ContinuityBaseline`（连续性基线） | Episode 使用的确切剧本/资产/风格/规则版本集合 | 新基线不静默覆盖历史镜头和审核快照 |
| `ContinuityCase`（连续性问题） | 带对象版本、规则、时码/区域证据和处置生命周期的问题 | 不直接修改 Shot；resolved 必须有修复版本和复检证据 |
| `EditLease`（编辑租约） | 对项目对象某编辑范围的有期限独占协调记录 | 不代替服务端权限和 expectedVersion 乐观并发校验 |
...
| 已关闭 | closed | 审核流程终结 |
| 已取消 | cancelled | 主动终止 |
| 策划中 | planning | 项目或 Episode 已建立但尚未进入正式制作 |
| 运行中 | running | 流水线/任务正在执行 |
| 已暂停 | paused | 流水线暂停，可恢复 |
| 已完成 | completed | 正常终结 |
| 已失败 | failed | 异常终结 |
| 制作中 | active | 项目或 Episode 正在正式生产；对话/模型配置语境显示为“活跃” |
...
| 已采纳 | adopted | 创作候选已被用户确认应用到剧本 |
| 已拒绝 | rejected_candidate | 创作候选被用户拒绝，任务终结 |
| 已过期 | expired | 创作候选超过 24h 未处理，自动过期 |
