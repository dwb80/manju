...
> **`published` 语义说明（成片已发布）**：本上下文中 `FinalVideo` 的 `published` 表示**成片已发布**——即成片已发布到外部平台（如 B 站/抖音），是发布交付流程的**终态**，关注的是"外发完成"。此语义与[资产库上下文](04-asset-library.md)中资产（角色/场景/道具）的 `published`（资产可被分镜引用的生命周期阶段、属中间态）**含义不同**，详见 [03-glossary.md "已发布"语义区分说明](../03-glossary.md)。
...
| 事件 | Payload | 消费者 |
|------|---------|-------|
| `FinalVideoCreated` | finalVideoId, projectId, episodeId, renderJobId, artifactRevision, artifactHash, renderSnapshotHash | 智能助手（成片列表与 DEP 制品链） |
...
| `FinalVideoPublished` | finalVideoId, projectId, artifactRevision, artifactHash, publishPlanId | 项目管控（更新项目进度）、智能助手（发布确认与 DEP 发布证据） |
...
| `PublishRecordCreated` | recordId, planId, finalVideoId, artifactRevision, platform, status, externalId, externalUrl, receiptHash | 智能助手（发布历史与 DEP 发布证据） |
