...
下表按 [DDD §1 限界上下文](ddd/domain-requirements-spec.md) 定义目标物理契约及所属上下文。表名出现在本文不代表已经实现；实际存在性和发布状态以 [feature-status.md](feature-status.md) 的代码核验证据为准。**表与领域实体的映射**见 [architecture-and-development.md §6.2](architecture-and-development.md)。
...
完整升级与回滚策略见 [release/v2-rollback.md](release/v2-rollback.md)。
