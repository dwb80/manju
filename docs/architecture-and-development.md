...
> **领域归属**：[上下文映射](ddd/context-map.md) + [公共领域基础设施](ddd/infrastructure.md) + [依赖方向约束](ddd/dependency-rules.md)

> 领域边界、聚合、事件、状态机以 [DDD 领域需求规格](ddd/domain-requirements-spec.md) 为权威来源。本文档描述**技术实现层**的架构、技术栈、项目结构、存储方案和开发排错指南。
...
| OpenAPI 3.0（目标门禁） | API 契约；生成和漂移校验按 [开发与交付规范](development-standards.md) |
| Pino | 结构化日志 |
...
> **物理表与领域实体的对应关系**见 §6。领域层术语（Project、Episode、ScriptDocument、Storyboard、Shot、Character、Scene、Prop、Asset、FinalVideo、PublishPlan、PipelineRun、Review、QCReport、WorkItem、Conversation）以 [DDD 文档](ddd/domain-requirements-spec.md) §2 为准。
...
统一 Definition of Done、覆盖率目标和测试分层见 [开发与交付规范](development-standards.md)。本节命令是当前项目入口，不降低该规范的质量门禁。
...
> 领域层的聚合边界、事件、命令、不变量见 [DDD 领域需求规格](ddd/domain-requirements-spec.md) §3。本节给出**领域实体到物理表**的映射，便于阅读代码与文档互相对照。
...
Shot 通过 `assetId + assetVersion` 引用 Character / Scene / Prop；EditProject 以 `sourceId + version` 引用分镜、音频和字幕。跨实体引用规则见 [依赖方向约束](ddd/dependency-rules.md)。
...
> **领域归属**：[上下文映射](ddd/context-map.md) + [模块-上下文映射表](ddd/module-map.md)
>
...
> **领域归属**：[公共领域基础设施](ddd/infrastructure.md) + [依赖方向约束](ddd/dependency-rules.md)
>
> 本节描述后端在数据写入、聚合更新、AI 任务执行等场景下的并发控制规范。领域层的乐观并发契约（`AggregateRoot.version`、`AggregateRepository.save(expectedVersion)`、`IdempotencyKeyProvider`）以 [公共领域基础设施](ddd/infrastructure.md) 为权威来源；本节描述技术实现层的落地方式。基础并发安全（WAL 模式、参数化语句、连接释放）见 [§4.5](#45-写入与并发安全)。
...
> **领域归属**：[公共领域基础设施](ddd/infrastructure.md) + [上下文映射](ddd/context-map.md)
>
> 本节描述后端 HTTP 层的统一错误处理与响应规范。项目不使用 Express / Koa 中间件链，而是在 Node 原生 HTTP 服务器中通过集中式错误处理函数 + 顶层 try-catch 兜底实现等价能力。领域错误码契约以 [公共领域基础设施](ddd/infrastructure.md) 为权威来源。
