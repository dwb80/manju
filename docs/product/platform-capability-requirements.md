...
> **权威性**：本文经 `requirements-and-acceptance.md` 引用后，作为 US-022～US-029 的详细验收附件。若摘要与本文冲突，以本文的可测试验收条件为准。  
...
- 默认目标：RPO ≤ 24h、RTO ≤ 4h；生产部署可配置更严格目标但不得放宽而不告警。
...
- 页面存在、接口存在或脚本存在均不能单独视为完成；必须回填追踪矩阵和 `feature-status.md`。
...
|---|---|---|
| 用户 | `POST /api/admin/users`、`POST /api/admin/invitations`、`PATCH /api/admin/users/{id}/status`、`POST /api/admin/users/{id}/revoke-sessions` | users, organization_memberships, invitations, sessions, sso_links |
| 本人会话 | `GET /api/auth/sessions`、`DELETE /api/auth/sessions/{id}`、`POST /api/auth/change-password` | sessions, auth_attempts |
| 通知 | `GET /api/notifications`、`POST /api/notifications/read-batch`、`POST /api/notifications/{id}/archive`、`GET/PUT /api/notification-preferences`、`GET /api/notifications/stream` | notifications, notification_preferences, notification_delivery_attempts, notification_templates |
| 回收站 | `GET /api/recycle-bin`、`POST /api/recycle-bin/restore-plan`、`POST /api/recycle-bin/restore`、`POST /api/recycle-bin/permanent-delete` | 各聚合 deleted_at + retention_holds + cleanup_batches |
| 配置 | `GET /api/settings/effective`、`GET/PUT/DELETE /api/settings/{scope}/{key}`；预算/质量/表现使用专用 API | typed_settings, setting_versions, project_budgets, quality_rule_sets |
| 审计 | `GET /api/audit-records`、`POST /api/audit-record-exports`、`GET /api/audit-record-exports/{id}` | audit_records, audit_export_jobs |
| 导入导出 | `POST /api/project-exports`、`GET /api/project-exports/{id}`、`POST /api/project-imports/plan`、`POST /api/project-imports/{id}/confirm` | export_jobs, import_jobs, import_object_mappings, staging_files |
| 备份恢复 | `POST /api/admin/backups`、`POST /api/admin/backups/{id}/verify`、`POST /api/admin/restores/plan`、`POST /api/admin/restores/{id}/execute` | backup_sets, backup_artifacts, restore_jobs, restore_verifications |
