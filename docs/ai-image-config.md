...
> **所属上下文**：[AI 任务调度 §3.5](ddd/contexts/05-ai-task-orchestration.md)
...
/**
 * 单一真相源：AI 生图所有参数
 * 修改前必须先更新 docs/ai-image-config.md
 */
...
- [x] §7.7 演进策略
- [x] §7.8 关键决策
- [x] 配套文档：`factories-assets-and-image-views.md §12.5/§12.6/§13` 已冻结
...
- [x] `factories-assets-and-image-views.md §13.2` 增"size 不字面量"提醒
...
- [ ] `factories-assets-and-image-views.md` 按钮文案改"📚 资产库 / 🎨 风格锚定"
- [ ] `factories-assets-and-image-views.md` 工厂模块移除所有 lucide SVG 图标
- [ ] `factories-assets-and-image-views.md` 取消 40 张上限；改 `backend/src/services/asset-image.service.ts` 删裁剪逻辑
- [ ] `factories-assets-and-image-views.md` §12.6.5 A↔B 双向导通
