...
- `backend/src/http/ai-tasks-router.ts`：AI 任务队列路由（任务管理、批量操作）。
- `backend/src/http/data-router.ts`：数据中心路由（成本统计、效率分析）。
- `backend/src/http/models-router.ts`：模型中心路由（模型列表、设置默认）。
- `backend/src/http/publish-router.ts`：发布中心路由（成片管理、发布计划）。
- `backend/src/http/pipeline-router.ts`：8 阶段生产流水线路由（状态机、阶段推断）。
- `backend/src/services/domain.ts`：核心业务逻辑，例如创建会话、生成图片、生成视频、项目管理。
- `backend/src/services/module-domain.ts`：角色/场景/道具/分镜/音频/视频/剪辑等模块业务逻辑。
- `backend/src/services/script-center-impl.ts`：剧本中心服务（剧本文档、剧集、场景、对白、AI 生成）。
...
- `backend/src/types.ts`：后端核心数据类型（兼容层，re-export 各子模块类型）。
...
- `frontend/app/ai-tasks/page.tsx`：AI 任务队列页面（真实 API 对接）。
- `frontend/app/data/page.tsx`：数据中心页面（真实 API 对接）。
- `frontend/app/models/page.tsx`：模型中心页面（真实 API 对接）。
- `frontend/app/publish/page.tsx`：发布中心页面（真实 API 对接）。
- `frontend/app/images/[id]/page.tsx`：图片详情页。
- `frontend/app/videos/[id]/page.tsx`：视频详情页。
- `frontend/components/dashboard/home-dashboard.tsx`：驾驶舱（项目进度、核心指标、8 阶段流水线）。
- `frontend/components/conversation-sidebar.tsx`：侧边栏导航（含 AI 任务、数据中心、模型中心、发布中心入口）。
