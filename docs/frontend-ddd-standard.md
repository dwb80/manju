...
> 权威归属：本文档为前端开发规范权威，领域定义以 `docs/ddd/` 为准，API 契约以 `docs/api.md` 为准。
...
UI 文案必须使用统一语言术语（见 `docs/ddd/glossary.md`），禁止使用禁止别名：
...
| 文档 | 关系 |
|------|------|
| `docs/ddd/` 全部 DDD 文档 | 领域权威，本文档的领域定义（聚合根、状态机、事件）以其为准 |
| `docs/architecture-and-development.md` §7 | 现有前端架构规范，本文档是其向 DDD 演进的升级版 |
| `docs/development-standards.md` §6 | 现有状态管理规范，本文档与其保持一致并补充领域层约束 |
| `docs/ddd/glossary.md` | 统一语言术语表，本文档强制引用 |
| `docs/ddd/dependency-rules.md` | 跨聚合引用规则，本文档前端实现须遵循 |
| `docs/ddd/contracts.md` | 防腐层与 CQRS 读模型定义，本文档基础设施层须遵循 |
