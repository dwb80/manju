...
> 共享规则见 [01-common-contract.md](01-common-contract.md)。本册覆盖 US-007～US-012。
...
| `POST /api/v1/scenes/{id}/publish`、`POST /api/v1/props/{id}/publish` | 发布当前草稿 | `asset_incomplete` |
...
- 资产中心提供风格资产列表、详情、版本对比、发布/归档和影响分析；展示参考图、色板、线条、光照、材质、正向/负向Prompt、来源和授权。
- 项目设置可选择一个已发布默认风格；Shot导演台显示最终生效风格和来源，覆盖前展示影响，不修改已冻结快照。
...
```gherkin
@US-030 @e2e @p0
Scenario: US-030-S01 发布项目默认风格不改写冻结快照
  Given 项目已有使用风格版本1的冻结快照
  When 美术发布风格版本2并设为项目默认
  Then 新任务解析到版本2而旧快照仍引用版本1
```
...
- 资产详情提供“复制到项目”，只显示操作者有写权限的目标项目；确认页展示版本、依赖、重名、授权和预计成本。
- 复制结果逐项显示成功/失败及目标资产链接；目标副本显示来源血缘但不自动同步。

### API 契约

| 方法与路径 | 请求/响应 | 约束与错误 |
|---|---|---|
| `POST /api/v1/asset-copy-plans` | `{sourceRefs[],targetProjectIds[]}`；返回冲突、依赖、授权和成本预览 | 来源读/导出权＋目标创建权 |
| `POST /api/v1/asset-copy-plans/{id}:confirm` | `{commandId,planDigest,resolutions[]}`；202批次 | plan变化拒绝；逐资产原子 |

### 可执行验收用例

```gherkin
@US-031 @integration @p1
Scenario: US-031-S01 复制资产形成独立版本和血缘
  Given 用户可读取来源资产版本3并可写目标项目
  When 用户确认复制计划
  Then 目标项目产生新资产版本1且记录来源版本3，来源后续修改不改变目标
```
...
```gherkin
@US-032 @e2e @p0
Scenario: US-032-S01 授权到期阻止待发布制品
  Given FinalVideo引用的声音授权已到期
  When 发布运营执行目标渠道预检
  Then 预检返回blocker、列出受影响AudioAsset版本且不创建执行中的PublishRecord
```
...
| `POST /api/v1/reviews/{id}/decisions` | `{commandId,decision,reasonCode,comment?}`，decision 为 approve 或 request_changes；201 decision | `If-Match`；`self_review_forbidden` |
...
- 参数、模式、能力协商、质量和批量规则以[AI视频生成参数与能力规范](../requirements/modules/07-ai-video-config.md)为权威。
